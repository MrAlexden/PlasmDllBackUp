...
extern "C" __declspec(dllexport) myflo *** Cilinder(vector <myflo> vPila, vector <myflo> vSignal, vector <myflo> AdditionalData)
{
	if (vPila.size() == 0
		|| vPila.empty()
		|| vSignal.size() == 0
	...
	-- fdata[3] - массив с параметрами
		--[i][j]
			...
			j::столбики, их всегда 5 для цилиндра
				--1. время
	...
	-- fdata[4] - массив из одного элемента из трех элементов
		--[0][i]
			...
			--3. столбики для fdata[3], их всегда 5 для цилиндра
	=============================================================================================*/
