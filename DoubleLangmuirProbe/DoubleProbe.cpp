...
int DoubleProbe(_In_ vector <myflo> & vRamp,
			    ...
			    _Out_ Plasma_proc_result <myflo> & fdata)
{
	...
	for (int segnum = 0; segnum < numSegments; ++segnum)
	{
		...
		/*if (make_one_segment(3, vSegRamp, vY, vres, vfilt, vdiff, vcoeffs) < 0)
			continue;*/
		make_one_segment(3, vSegRamp, vY, vres, vfilt, vdiff, vcoeffs);
