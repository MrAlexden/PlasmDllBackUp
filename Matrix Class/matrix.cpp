...
Matrix::Matrix(unsigned rowSize, unsigned colSize, myflo initial){
    m_rowSize = rowSize;
    m_colSize = colSize;
...
// Scalar Addition
Matrix Matrix::operator+(myflo scalar){
...
myflo& Matrix::operator()(const unsigned &rowNo, const unsigned & colNo)
{
...
//Second: Eigen Value
//Third: Flag
tuple<Matrix, myflo, int> Matrix::powerIter(unsigned rowNum, myflo tolerance){
    ...
    int errorCode = 0;
    myflo difference = 1.0; // Initiall value greater than tolerance
    unsigned j = 0;
    unsigned location;
    // Defined to find the value between last two eigen values
    vector <myflo> eigen;
    myflo eigenvalue = 0.0;
    eigen.push_back(0.0);
...
Matrix Matrix::deflation(Matrix &X, myflo &eigenvalue)
{
    // Deflation formula exactly applied
    myflo denominator = eigenvalue / (X.transpose() * X)(0,0);
